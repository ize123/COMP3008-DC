    ...
    internal class DataServer : ServerInterface
    {
        ...
        public void GetValuesForEntry(int index, out uint acctNo, out uint pin, out int bal, out string fName, out string lName, out MemoryStream image)
        {
            ...
            catch(ArgumentOutOfRangeException e)
            {
                ...
                throw new FaultException<ServerError>(se, "ERROR: " + e.Message + "\nMIN = 1, MAX = " + GetNumEntries());
            } 
        } 

        public DataStruct Search(string searchString)
        {
            List<DataStruct> students = dbl.GetAllStudents();
            DataStruct student = new DataStruct();
            var match = students.FirstOrDefault(x => x.lastName.ToLower().Equals(searchString.ToLower()));
            if (match != null)
            {
                int index = students.IndexOf(match);
                student = dbl.GetStudentByIndex(index);
            }
            else
            {
                student = null;
